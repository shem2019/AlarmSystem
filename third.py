...
duration = 12  # Duration to listen to microphone (in seconds)
...
def listen_and_classify_mic():
    ...
    if predicted_class is not None:
        ...
        print(f"Predicted sound from microphone: {class_name}")
    else:
...
def classify_file():
    ...
    if file_path:
        ...
        if predicted_class is not None:
            class_name = class_names.get(predicted_class, "Unknown")
            result_var.set(f"Predicted sound: {class_name}")
            print(f"Predicted sound from file {file_path}: {class_name}")
        else:
